...
| **`ForwardN`** | **`1`** (по умолчанию) — после обработки строка уходит в штатный **`SendCommand`**; **`0`** — перехват без отправки на сервер. |

**Форматы `ColorN`** (как у цвета игрока / бабла, **D3DCOLOR ARGB**):

- Целое **со знаком в десятичной записи**, например **`-6684673`** — биты трактуются как у 32-битного цвета в SA:MP.
- **`{RRGGBB}`** или **`{RRGGBBAA}`** в hex (регистр не важен); для 6 символов альфа **`FF`**.
- **`0x` + 8 hex** — явный **ARGB** (`0xFFFF99FF` и т.п.).
...
1. **MinHook** на **`localPlayerChatOffset`** — только при **`MirrorOwnChatBubble=1`** (исходящий текст чата без ведущего **`/`**). Если оффсет **`0`** (у **R2** в таблице), хук не ставится.
