...
3. Если рядом с ASI **нет** **`tagOnPlayer.ini`**, при первом чтении конфига создаётся файл по умолчанию: **`[Settings]`** (в т.ч. **`MirrorOwnChatBubble=1`**) и **`[OverlayCommands]`** с **14** типовыми командами (у **`/me`** и **`/todo`** — шаблоны бабла из кода). Если файл уже есть, плагин его не перезаписывает целиком; секция **`[Settings]`** по-прежнему обновляется актуальными значениями при старте.
...
Остальные символы в шаблоне копируются как есть (пробелы, `*`, текст). В итоговую строку подставляются литералы **`{RRGGBB}`**; несколько цветов в одном бабле зависят от клиента SA:MP. Если вставки **`{RRGGBB}`** не разбираются, используется базовый цвет из **`ColorN`** в **`Add`**.
